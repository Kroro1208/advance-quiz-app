export const categoryTranslationMap: { [key: string]: string } = {
  "General Knowledge": "一般知識",
  "Entertainment: Books": "エンターテイメント: 本",
  "Entertainment: Film": "エンターテイメント: 映画",
  "Entertainment: Music": "エンターテイメント: 音楽",
  "Entertainment: Musicals & Theatres":
  "エンターテイメント: ミュージカル & 劇場",
  "Entertainment: Television": "エンターテイメント: テレビ",
  "Entertainment: Video Games": "エンターテイメント: ビデオゲーム",
  "Entertainment: Board Games": "エンターテイメント: ボードゲーム",
  "Science & Nature": "科学 & 自然",
  "Science: Computers": "科学: コンピュータ",
  "Science: Mathematics": "科学: 数学",
  "Mythology": "神話",
  "Sports": "スポーツ",
  "Geography": "地理",
  "History": "歴史",
  "Politics": "政治",
  "Art": "アート",
  "Celebrities": "有名人",
  "Animals": "動物",
  "Vehicles": "乗り物",
  "Entertainment: Comics": "エンターテイメント: コミック",
  "Science: Gadgets": "科学: ガジェット",
  "Entertainment: Japanese Anime & Manga":
    "エンターテイメント: 日本のアニメ & マンガ",
  "Entertainment: Cartoon & Animations":
    "エンターテイメント: カートゥーン & アニメーション",
};
